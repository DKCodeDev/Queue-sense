...
def create_service(current_user):
    ...
    existing = Service.query.filter_by(service_code=data['service_code']).first()
    
    if existing:
        return jsonify({
            'error': 'Service code already exists',
            'message': f"The code '{data['service_code']}' is already in use"
        }), 409
...
def delete_service(current_user, service_id):
    ...
    # Delete service (cascades to locations)
